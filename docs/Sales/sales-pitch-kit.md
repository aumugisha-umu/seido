...
```
Declencheur (appel, email, WhatsApp, visite)
...
| Phase | Nom | Statut | Ce que fait SEIDO |
|-------|-----|--------|-------------------|
| 1 | **Le squelette** | En cours | Centralisation de toutes les donnees, documents et interactions. Gain de temps immediat par la recherche instantanee et le transfert en quelques clics. |
| 2 | **Les reflexes** | Partiellement en place | Automatisations deterministes : notifications, rappels, alertes echeances, workflows pre-configures. Tout ce qui est previsible est automatise. |
| 3 | **Le cerveau** | Roadmap | Agent IA qui pre-mache le travail : analyse des emails entrants, propositions de reponses, regularisation des charges, traitement des demandes administratives. L'utilisateur confirme ou reprend la main. |
...
Tout commence par notre application, sur laquelle vous centralisez toutes vos donnees, documents et interactions liees a votre patrimoine. Biens, lots, contrats, contacts, conversations — tout est au meme endroit.

Ensuite, notre systeme s'occupe de prendre en main le flux. A chaque email, WhatsApp ou meme appel recu, vous etes notifie uniquement quand une decision vous revient. Vous avez directement tout le contexte — l'historique, les documents, les echanges — pour agir en quelques clics et passer a autre chose.

Vos locataires suivent leurs demandes comme un colis, sans avoir besoin de vous appeler. Vos prestataires recoivent toutes les informations avant de se deplacer et gerent leur planning de facon autonome. Et vous, vous pilotez au lieu d'eteindre des feux.

**La ou ca devient puissant**

Aujourd'hui, on construit le squelette — le logiciel de gestion qui centralise et fluidifie tout. Et on a deja mis en place des automatisations deterministes : notifications intelligentes, rappels d'echeances, workflows pre-configures, rappels de loyer. Ca, ca fait deja gagner enormement de temps a nos utilisateurs.
...
## Pitch 5 minutes — Full story

> *~750 mots. Pour : rendez-vous investisseur, presentation partenaire, pitch competition.*

**L'observation de depart**
...
SEIDO est une plateforme sur laquelle vous centralisez toutes vos donnees, documents et interactions liees a votre patrimoine. Biens, lots, contrats, contacts, emails, conversations — tout est dans un seul endroit, accessible en quelques secondes.
...
Pour arriver a cette ambition de 10 heures ramenees a 1, on procede par etapes.

*Phase 1 — Le squelette.* C'est ce qu'on construit aujourd'hui : le logiciel de gestion qui centralise et fluidifie tout. On travaille avec des partenaires terrain — des gestionnaires, des syndics — qui nous aident a comprendre ce qui est possible d'automatiser, ce qui apporte le plus de valeur.

*Phase 2 — Les reflexes.* C'est deja partiellement en place. Tout ce qui est deterministe et previsible, on l'automatise : notifications intelligentes, rappels d'echeances, alertes de retard, workflows pre-configures, rappels de loyer. Ces automatisations font deja gagner enormement de temps a nos utilisateurs, sans qu'ils aient a y penser.

*Phase 3 — Le cerveau.* C'est la ou ca devient vraiment puissant. On va ajouter un agent intelligent qui premache le travail. Concretement : un email arrive, l'agent identifie le sujet, retrouve le dossier concerne, analyse les donnees pertinentes, et prepare une proposition d'action — une reponse d'email, une mise a jour de statut, une relance. L'utilisateur n'a qu'a confirmer ou reprendre la main.

Que ce soit pour une regularisation de charges, un traitement de demande administrative, un suivi de prestataire, ou une reponse a une question d'un locataire — l'agent fait le travail d'analyse sur des donnees exactes et presente l'action a valider. Et avec le temps, plus il apprend de vos habitudes, plus il peut aller loin en autonomie.
...
## Comparaison avec l'ancien positionnement
