/**
 * Version démo de InterventionsSectionWithModals
 * Utilise les hooks démo au lieu des hooks production
 */
...
import { useDemoInterventionApproval } from "@/hooks/demo/use-demo-intervention-approval"
import { useDemoInterventionQuoting } from "@/hooks/demo/use-demo-intervention-quoting"
import { useDemoInterventionPlanning } from "@/hooks/demo/use-demo-intervention-planning"
import { useDemoInterventionExecution } from "@/hooks/demo/use-demo-intervention-execution"
import { useDemoInterventionFinalization } from "@/hooks/demo/use-demo-intervention-finalization"
...
import { InterventionsSectionClient } from "@/app/gestionnaire/dashboard/interventions-section-client"
...
interface DemoInterventionsSectionWithModalsProps {
  interventions: any[]
  totalCount: number
  teamId: string
}

export function DemoInterventionsSectionWithModals({
  interventions,
  totalCount,
  teamId
}: DemoInterventionsSectionWithModalsProps) {
  ...
  // ✅ Demo action hooks (remplacent les hooks production)
  const approvalHook = useDemoInterventionApproval()
  const quotingHook = useDemoInterventionQuoting(teamId)
  const planningHook = useDemoInterventionPlanning()
  const executionHook = useDemoInterventionExecution()
  const finalizationHook = useDemoInterventionFinalization()

  // Fetch quote requests when programming modal opens (DEMO - depuis le store)
  useEffect(() => {
    const fetchQuoteRequests = async () => {
      if (planningHook.programmingModal.isOpen && planningHook.programmingModal.intervention?.id) {
        // En mode démo, récupérer depuis le store
        const quotes = store.query('intervention_quotes', {
          filters: { intervention_id: planningHook.programmingModal.intervention.id }
        })

        // Enrichir avec les données du provider
        const quotesWithProvider = quotes.map((quote: any) => {
          const provider = store.get('users', quote.provider_id)
          return {
            ...quote,
            provider
          }
        ...
        setQuoteRequests(quotesWithProvider as Quote[])
      } else {
        setQuoteRequests([])
      }
    }
    ...
    fetchQuoteRequests()
  }, [store, planningHook.programmingModal.isOpen, planningHook.programmingModal.intervention?.id])
  ...
  // Confirm cancel quote request (DEMO - simulé)
  const handleConfirmCancelQuote = async () => {
    ...
    try {
      ...
      // TODO: En mode démo, supprimer du store
      // store.delete('intervention_quotes', cancelQuoteModal.quoteId)
      ...
      // ✅ DEMO MODE: No need to refresh - data is in-memory (no server state to sync)
    } catch (error) {
  ...
  return (
    <InterventionCancellationProvider>
      ...
      <QuoteRequestSuccessModal
        ...
        providerNames={quotingHook.successModal.providerNames}
      ...
      <ProgrammingModal
        ...
        onSubmit={planningHook.handleProgrammingConfirm}
        isFormValid={planningHook.isProgrammingFormValid()}
        quoteRequests={quoteRequests}
        onViewProvider={(providerId) => router.push(`/demo/gestionnaire/contacts?highlight=${providerId}`)}
