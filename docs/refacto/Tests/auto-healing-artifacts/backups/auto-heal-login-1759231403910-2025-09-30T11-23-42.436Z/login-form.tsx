...
export function LoginForm() {
  ...
  // ✅ PATTERN OFFICIEL SUPABASE SSR: Navigation client-side après authentification
  // Ceci résout le problème de race condition avec les cookies Supabase
  useEffect(() => {
    if (state.success && state.data?.redirectTo) {
      console.log('✅ [LOGIN-FORM] Authentication successful, navigating to:', state.data.redirectTo)
      // Attendre un peu pour garantir la propagation des cookies avant navigation
      setTimeout(() => {
        router.push(state.data.redirectTo as string)
        router.refresh() // Force le refresh pour mettre à jour le middleware
      }, 500) // 500ms est suffisant pour la propagation côté client
    }
  }, [state, router])
