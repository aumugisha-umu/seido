...
export interface DocumentFileWithPreview<T extends string = string> {
  id: string
  file: File
  preview?: string
  progress: number
...
export interface DocumentTypeOption<T extends string = string> {
  value: T
  label: string
}

interface DocumentFileAttachmentProps<T extends string = string> {
  files: DocumentFileWithPreview<T>[]
  documentTypes: DocumentTypeOption<T>[]
  onAddFiles: (files: File[]) => void
  onRemoveFile: (fileId: string) => void
  onUpdateFileType: (fileId: string, documentType: T) => void
  isUploading?: boolean
  className?: string
  maxFiles?: number
  accept?: string
...
export function DocumentFileAttachment<T extends string = string>({
  files,
  documentTypes,
  onAddFiles,
  onRemoveFile,
  ...
  hint = 'Déposez des fichiers ici ou cliquez sur le bouton ci-dessus'
}: DocumentFileAttachmentProps<T>) {
  ...
  const getStatusBadge = (file: DocumentFileWithPreview<T>) => {
    switch (file.status) {
      case 'uploading':
        return (
  ...
  const getFileUrl = (file: DocumentFileWithPreview<T>) => {
    ...
    <div className={`flex flex-col h-full ${className}`}>
      ...
      {files.length > 0 && (
        <div className="mt-4 flex-1 min-h-0 flex flex-col max-w-full">
          <div
            className={`flex gap-3 max-w-full ${files.length > 1 ? 'overflow-x-auto pb-2' : ''}`}
            onDrop={handleDrop}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="w-full">
                        <Select
                          ...
                          onValueChange={(value) => onUpdateFileType(file.id, value as T)}
                          disabled={isUploading}
                        >
