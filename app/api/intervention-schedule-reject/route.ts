...
export async function POST(request: NextRequest) {
  console.log("❌ intervention-schedule-reject API route called")
  ...
  try {
    ...
    const { interventionId, rejectionReason } = body
    ...
    if (!rejectionReason || !rejectionReason.trim()) {
      return NextResponse.json({
        success: false,
        error: 'Le motif du refus est requis'
      }, { status: 400 })
    }
    ...
    if (user.role !== 'prestataire') {
      return NextResponse.json({
        success: false,
        error: 'Seuls les prestataires peuvent refuser un planning'
      }, { status: 403 })
    }
    ...
    if (intervention.status !== 'planification') {
      return NextResponse.json({
        success: false,
        error: `Le planning ne peut être refusé que pour une intervention en planification (statut actuel: ${intervention.status})`
      }, { status: 400 })
    }
    ...
    // Check if there's a scheduled date to reject
    if (!intervention.scheduled_date) {
      return NextResponse.json({
        success: false,
        error: 'Aucune date planifiée à refuser'
      }, { status: 400 })
    }

    console.log("🔄 Storing rejection reason...")

    // Update intervention with rejection details
    // Status stays as 'planification' - gestionnaire will need to replan
    const { data: updatedIntervention, error: updateError } = await supabase
      ...
      .eq('id', interventionId)
      .select('id, status, title, rejection_reason, updated_at')
      .single()
    ...
    console.log("✅ Schedule rejected - intervention stays in planification")
    ...
    if (managerContact?.user_id) {
      try {
        await notificationService.create({
          ...
          type: 'intervention_schedule_rejected',
          title: 'Planning refusé par le prestataire',
          message: `Le prestataire a refusé le planning proposé pour l'intervention "${intervention.title}". Motif: ${rejectionReason.trim()}`,
          link: `/gestionnaire/interventions/${interventionId}`,
          metadata: {
            intervention_id: interventionId,
            rejection_reason: rejectionReason.trim(),
            rejected_date: intervention.scheduled_date,
            rejected_by: user.id
          }
        })
        console.log("📧 Manager notified of schedule rejection")
      } catch (notifError) {
    ...
    return NextResponse.json({
      ...
      intervention: {
        ...
        rejection_reason: updatedIntervention.rejection_reason,
    ...
    console.error("❌ Error in intervention-schedule-reject API:", error)
    ...
    return NextResponse.json({
      success: false,
      error: 'Erreur lors du refus du planning'
    }, { status: 500 })
  }
}
