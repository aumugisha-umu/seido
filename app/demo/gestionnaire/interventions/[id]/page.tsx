/**
 * Page Détail Intervention Gestionnaire - Mode Démo
...
function getStatusLabel(status: string): { label: string; color: string } {
  const statusLabels: Record<string, { label: string; color: string }> = {
    'demande': { label: 'Demande', color: 'bg-blue-100 text-blue-800' },
    'rejetee': { label: 'Rejetée', color: 'bg-red-100 text-red-800' },
    'approuvee': { label: 'Approuvée', color: 'bg-green-100 text-green-800' },
    'demande_de_devis': { label: 'Demande de devis', color: 'bg-yellow-100 text-yellow-800' },
    'planification': { label: 'Planification', color: 'bg-purple-100 text-purple-800' },
    'planifiee': { label: 'Planifiée', color: 'bg-indigo-100 text-indigo-800' },
    'en_cours': { label: 'En cours', color: 'bg-orange-100 text-orange-800' },
    'cloturee_par_prestataire': { label: 'Clôturée (prestataire)', color: 'bg-teal-100 text-teal-800' },
    'cloturee_par_locataire': { label: 'Clôturée (locataire)', color: 'bg-cyan-100 text-cyan-800' },
      ...
      <Card className="bg-blue-50 border-blue-200">
        <CardContent className="pt-6">
          <p className="text-sm text-blue-800">
            ...
            (Documents, Chat, Devis, etc.) sera ajoutée prochainement.
          </p>
        </CardContent>
      </Card>
    </div>
