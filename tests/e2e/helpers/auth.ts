 ...
 *
 * With globalSetup, cookies are already loaded by newPage().
 * This function verifies auth by navigating to the dashboard.
 * Falls back to full login if cookies are expired/missing.
 *
 * @param page - Puppeteer page (cookies already loaded by newPage)
...
export async function login(page: Page, role: string = 'gestionnaire'): Promise<TestAccount> {
  ...
  if (url.includes('/auth/login')) {
    console.warn('[Auth] Cookies expired — performing full login')
