...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (files && files.length > 0) {
      console.log(`📎 [UPLOAD-DEBUG] Processing ${files.length} file(s) for intervention...`)
      console.log(`📎 [UPLOAD-DEBUG] Intervention ID: ${intervention.id}`)
      console.log(`📎 [UPLOAD-DEBUG] Files details:`, files.map(f => ({ name: f.name, size: f.size, type: f.type })))
      ...
      try {
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          try {
            ...
            console.log(`✅ [UPLOAD-DEBUG] File uploaded successfully: ${file.name}`)
            console.log(`✅ [UPLOAD-DEBUG] Upload result:`, uploadResult)
            uploadedDocuments.push(uploadResult.document)
            filesUploaded++

          } catch (fileError) {
            console.error(`❌ [UPLOAD-DEBUG] Error uploading file ${file.name}:`, fileError)
