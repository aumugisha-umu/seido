...
export async function loginAction(prevState: AuthActionResult, formData: FormData): Promise<AuthActionResult> {
  ...
  try {
    ...
    if (userResult.success && userResult.data && userResult.data.role) {
      ...
      console.log('🔄 [LOGIN-ACTION] Will redirect to role-specific dashboard:', {
  ...
  // ✅ PATTERN SUPABASE SSR FIX: Attendre que les cookies HTTP soient propagés
  // Les cookies Supabase prennent du temps à être écrits après signInWithPassword()
  // Si on redirect() immédiatement, le middleware ne verra pas la session
  // Solution : Attendre 1500ms pour garantir la propagation des cookies
  console.log('⏳ [LOGIN-ACTION] Waiting for session cookies propagation before redirect...')
  await new Promise(resolve => setTimeout(resolve, 1500))
  console.log('✅ [LOGIN-ACTION] Session cookies propagated, redirecting now...')
