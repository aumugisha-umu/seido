...
export async function loginAction(prevState: AuthActionResult, formData: FormData): Promise<AuthActionResult> {
  ...
  // Au lieu de redirect() qui crée une race condition avec les cookies,
  // on retourne le dashboardPath pour que le client gère la navigation
  // MAIS on doit quand même attendre que les cookies soient propagés côté serveur
  // sinon le middleware ne verra pas la session lors du retour de l'action
  console.log('⏳ [LOGIN-ACTION] Waiting for server-side cookie propagation...')
  await new Promise(resolve => setTimeout(resolve, 1000))
  console.log('✅ [LOGIN-ACTION] Authentication successful, returning dashboard path for client-side navigation')
