...
describe('E2E: Read-Only Enforcement', () => {
  let page: Page
  const baseUrl = getBaseUrl()

  beforeAll(async () => {
    if (!READ_ONLY_E2E) {
      console.log('⏭️  Set READ_ONLY_E2E=true and configure subscription to read_only before running')
    }
    page = await newPage()
  })
  ...
  it('read-only banner is visible on dashboard', async () => {
    if (!READ_ONLY_E2E) return
  ...
  it('read-only banner is not dismissible', async () => {
    if (!READ_ONLY_E2E) return
  ...
  it('biens page has disabled creation buttons', async () => {
    if (!READ_ONLY_E2E) return

    await page.goto(`${baseUrl}/gestionnaire/biens`, {
      waitUntil: 'domcontentloaded',
      timeout: 30_000,
    })
  ...
  it('interventions page has disabled creation buttons', async () => {
    if (!READ_ONLY_E2E) return

    await page.goto(`${baseUrl}/gestionnaire/interventions`, {
      waitUntil: 'domcontentloaded',
      timeout: 30_000,
    })
    await new Promise(resolve => setTimeout(resolve, 2000))
  ...
  it('read-only banner has reactivation CTA', async () => {
    if (!READ_ONLY_E2E) return
  ...
  it('export functionality still works in read-only mode (RGPD)', async () => {
    if (!READ_ONLY_E2E) return
