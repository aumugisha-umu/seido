/**
 * Page Détail Intervention Locataire - Mode Démo
...
function getStatusLabel(status: string): { label: string; color: string } {
  const statusLabels: Record<string, { label: string; color: string }> = {
    'demande': { label: 'En attente', color: 'bg-yellow-100 text-yellow-800' },
    'rejetee': { label: 'Rejetée', color: 'bg-red-100 text-red-800' },
    'approuvee': { label: 'Approuvée', color: 'bg-green-100 text-green-800' },
    'demande_de_devis': { label: 'Demande de devis', color: 'bg-blue-100 text-blue-800' },
    ...
    'cloturee_par_locataire': { label: 'Validée', color: 'bg-green-100 text-green-800' },
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2">
            ...
            {intervention.created_at && (
              <div>
                <p className="text-sm font-medium text-muted-foreground">Demandée le</p>
      ...
      <Card className="bg-blue-50 border-blue-200">
        <CardContent className="pt-6">
          <p className="text-sm text-blue-800">
            ...
            (Documents, Messages, etc.) sera ajoutée prochainement.
          </p>
        </CardContent>
      </Card>
    </div>
